...
public class Server_GUI extends Frame {
	...
	public Server_GUI() {
		...
		this.setTitle("ADRELI - Server"); // Fesntertitel
	...
	/**
	 * Dauer der Server Funktion an die Logausgabe anhängen.
	 */

	public static void addTime(long t) {

		String bisher = log.getText();
		log.setText(bisher + " (" + t + " ms)");
	}
	...
	/**
	 * Server wird gestartet. Eingabe des Ports, außerdem Wahl von adrelibase.csv
	 * und Datenbasis.
	 */
	...
	public void starten() {
		...
		port_l.setBounds(75, 100, 150, 20);
		port_l.setVisible(true);
		this.add(port_l);
		// TextField Port
		final TextField port_t = new TextField();
		port_t.setBounds(225, 100, 120, 20);
		...
		datenbasis_l.setBounds(75, 125, 150, 20);
...
*/
		...
		knopf.setBounds(225, 225, 125, 25);
	 ...
	 * keine Datei gewählt worden sein, wird adrelibase.csv verwendet.
	 */
	...
	public void importieren() {
		...
		if (!(chosenFile == null)) {
			if (fd.getFile()
					...
					.equals("csv")) {
				Server_Controller.importieren(chosenDir, chosenFile);
			} else {
